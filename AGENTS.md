...
**Style-board feedback loop:** before each style-board iteration, read [Simon's Notes - style boards](<docs/02-design/mockups/2026-09-19-style-boards/Notes - style boards.md>). Preserve his notebook and use its latest iteration feedback. Save each new set under its own `iteration-N/` folder, with local embeds and prompts; keep the [iteration index](docs/02-design/mockups/2026-09-19-style-boards/README.md) current.

**Mockup-only workflow override, Simon, 2026-09-19:** work directly in the primary root checkout on `main` for the current mockup iteration phase. No additional worktrees, task branches, commits/pushes or GitHub PR/merge ceremony is required for these local rounds. Preserve unrelated edits and keep artifacts embedded in the existing docs vault. [Mockup gallery](docs/02-design/mockups/README.md) owns filing; [Git workflow exception](docs/10-execution/git-workflow.md#mockup-only-local-work--2026-09-19) bounds this override. Code or broader changes retain the normal workflow.
...
Use [granny-session-lifecycle](.agents/skills/granny-session-lifecycle/SKILL.md) at change-session start/handoff and for cockpit or vault-visibility maintenance. CI checks scoped session coverage; the [guide](docs/10-execution/cockpit-guide.md#automated-maintenance-contract) owns commands and the narrowly bounded README exemption. Simon granted standing main-integration authority for completed documentation-only tasks on 2026-09-17: use a reviewed task branch and PR, merge after required checks pass, then verify/synchronize the primary checkout when safe. Pushing a worktree branch alone does not update the Obsidian vault.
...
Simon selected [Explicit Scroll Row](docs/02-design/mockups/2026-09-19-harbour-blue-home/selected-explicit-scroll-row/README.md) as the Home composition to use and develop in future sessions on 2026-09-19 under [ADR-0016](docs/09-decisions/ADR-0016-explicit-home-room-row.md). Preserve the stable Round composer, compact continuation when useful, open unframed room portraits, written Previous/Next for true overflow and direct See all rooms. Do not reopen earlier Home directions or replace the row with a tile grid without a new explicit decision. Implementation, responsive/accessibility evidence and persistent Room data remain separately gated.
...
For every change task, follow the [Git worktree and publication workflow](docs/10-execution/git-workflow.md): use a dedicated worktree and short-lived task branch, validate, stage only scoped work, commit and push to the verified Pueblo98/Granny origin, then verify remote SHA equals task HEAD. Simon explicitly authorized future task-branch pushes on 2026-09-14. On 2026-09-17 he additionally authorized documentation-only tasks to create a PR, wait for required checks, merge normally to `main`, and safely fast-forward the primary vault without repeated confirmation. Direct commits to `main` remain disallowed; mixed code/documentation work and releases/dependencies retain separate integration authority. Review-only tasks remain read-only. Preserve user work; never force-push, rewrite published history, stash/overwrite concurrent work or publish secrets/personal data/private Obsidian state. If dirty overlapping primary paths block synchronization, leave them untouched, report remote-main integration and the local-vault blocker, and do not copy files between worktrees.
