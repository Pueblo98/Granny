...
Brand foundation → identity territories → Simon review + accessibility/user evidence → selected proposed direction → validated semantic tokens and logo assets → Figma components/prototypes → explicit acceptance recorded in Git → Android implementation.
...
No production token file is generated in this planning mission. Next token task must specify schema, units (Android dp/sp vs Figma logical units), theme modes, font weight mapping, fallback and migration. No API secrets, paid font binaries without rights, private Figma credentials or device-specific assets.
