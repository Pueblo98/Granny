...
Local components: app shell, voice wrapper, session/cancel state, scope/permission monitor, observer, executor, policy/registry, verifier, explicit memory, Context Room index/resolver and minimal audit. The room layer stores versioned references and provenance, ranks current-room then relevant cross-room context, and never becomes execution authority. Optional provider transport is behind a replaceable adapter and local egress filter. MVP has no family backend or memory sync. V1 proposes Context Rooms and helper proposals after their separate evidence/admission; remote screen/control is deferred.
...
No vendor/framework/database is selected. No root, device-owner, system signature, OEM service, AOSP or custom hardware is assumed. The first authority slice [T-103](docs/10-execution/backlog.md#t-103) uses fake adapters and synthetic evidence; the next experience/contract slice [T-119](docs/10-execution/backlog.md#t-119) uses fictional room data and no production persistence.
