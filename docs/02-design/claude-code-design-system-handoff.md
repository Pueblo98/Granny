...
Keep MVP and App V1 distinctions in specimen annotations; ADR-0012 memory/adaptation must not silently enter MVP. Open Home/persistence and agent-generated-interface choices do not block a provisional token/component system, but must not be decided by it.
