...
Keep MVP and App V1 distinctions in specimen annotations; ADR-0012 memory/adaptation must not silently enter MVP. Home/persistence remains open and does not block a provisional token/component system. [ADR-0013](../09-decisions/ADR-0013-bounded-interface-composition.md) accepts bounded semantic composition. Include component schema/version, allowed typed slots/actions, focus/source order and reflow constraints in the registry; unknown types and arbitrary executable controls fail closed. Stable Stop and input anchors remain renderer-owned. The system must not decide Home persistence.
