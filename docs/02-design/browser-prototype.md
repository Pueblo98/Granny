...
All runtime data is tab memory. The working thread is ephemeral; recent activity retains at most 20 task-kind/outcome summaries without message bodies or people. Preferences and aliases require explicit actions. Clear conversation, clear history, alias deletion and full reset have distinct scopes; reload clears runtime state. No IndexedDB, local/session storage, service worker, external fonts/images/scripts, provider connection or telemetry is added.
...
User text is rendered as text, never executable HTML/instructions. CSP blocks connections/media/fonts; response headers disable microphone/camera/geolocation. The loopback server has an exact runtime-file allowlist and rejects other files and write methods; [server tests](../../prototypes/stage-1/serve.test.mjs) enforce it.
