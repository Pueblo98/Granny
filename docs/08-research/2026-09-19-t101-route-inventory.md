...
**Scaffold status, 2026-09-19:** Simon authorized source preparation only. The [lab-only Android 16 scaffold](../../experiments/c2-screen-explanation/README.md) now contains separate synthetic fixture/observer modules, pure state/interpreter tests and an exact future build/install/run/teardown plan. It has not been built, installed or executed because the host has no Java/Android/Gradle/ADB toolchain and dependency installation/device authority were not granted. Current API review also leaves selected-package identity unresolved: MediaProjection supplies chosen content but the API 36 observer does not independently receive the selected package. A fixture marker therefore cannot satisfy the full source-identity oracle by itself.
