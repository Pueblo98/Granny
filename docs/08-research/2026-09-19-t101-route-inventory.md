...
| Requested media | Android package-targeted play-from-search **Go for lab handoff only / No-go as completion**; Spotify SDK **No-go**; Apple MusicKit/owner session **Unknown/Conditional** | Standard-intent handoff or fixed recipe **Conditional** | No — no named handler/device/provider run or independent playback oracle |
| Granny-owned text | Own-app preference **Go** | Not needed | No — internal setting |

The standard Android intent removes the earlier assumption that the next media experiment must begin with a provider SDK. The kill criterion is **not resolved**: scoped screen explanation is not admitted, the new media slice proves at most an external handoff, sent-photo retrieval has no present candidate route, message preparation/handoff cannot count as send and text adjustment is internal. Run the narrow T-124 source/host experiment first, then decide whether exact-handler device and provider-policy evidence can yield an independent playback oracle. If it cannot, Simon must still choose a policy-compatible owner integration, revise the outcome, replace a workflow or revisit ADR-0009's bar.
...
### C4 — media handoff and named integration (source/host slice active; device run not authorized)

- **Execution status:** [T-124](../10-execution/backlog.md#t-124) may implement the package-targeted Android play-from-search adapter in a debug/synthetic build with fake handler tests. No tablet, account, provider app, personal query or device action is authorized. Do not run a Spotify SDK/account experiment under the current product scope.
