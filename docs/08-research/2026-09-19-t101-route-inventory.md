...
The repaired C2 artifacts passed 29 host cases and a separately authorized device run. That run closes only the bounded protected-scene freshness regression and demonstrates the visible Stop control. It also exposes the next blockers: add explicit screen-off cleanup, explicit task-removal cleanup and content-free result/session-state recovery across activity recreation before requesting another device run. Selected-package identity and independent retention/egress evidence remain unresolved. C5 is lower risk but does not reduce the two-external-workflow kill risk. Each other C1–C5 experiment still requires its own device/build/fixture/account/grant/network/actuation/retention authority. T-104 remains blocked until the sent-photo route and applicable device-safety boundary have actual evidence.
