...
**Workflow disposition:** there is currently **no named policy-compatible media candidate ready for a device spike**. Spotify remains technically concrete but is policy no-go for the accepted voice-first behavior. Apple MusicKit is a desk lead only. Generic Media3 is conditional only where a named owner app exposes and accepts the route; launch/search alone is no-go as completion. Selecting a different provider or changing the voice-first outcome is a Simon decision, not an evidence-session inference.
...
| Requested media | Spotify **No-go** for current voice-assistant scope; Apple MusicKit/other named owner **Unknown/Conditional**; launch/search **No-go** as completion | Fixed recipe **Conditional** fallback | No — no policy-compatible named route or device/provider run |
| Granny-owned text | Own-app preference **Go** | Not needed | No — internal setting |

The accepted kill criterion has reached its **explicit revisit point for the current named route plan**: scoped screen explanation is the only remaining externally delegated path with bounded positive device observations, but it is not admitted; Spotify is policy-incompatible with the accepted voice-first media behavior, and no replacement named media route is ready. Sent-photo retrieval has no present candidate route, message preparation/handoff cannot count as send, and text adjustment is internal. This does not itself revise ADR-0009 or end Stage 1. Simon must now choose whether to investigate a policy-compatible provider such as the Apple MusicKit desk lead, change the media outcome, replace a workflow, or revise the two-external-workflow bar—without weakening evidence labels or beginning Stage 2.
...
### C4 — named media integration (frozen pending route decision)

- **Execution status:** do not run a Spotify account/device experiment under the current product scope. Select and review a policy-compatible named provider first; a controlled Media3 fixture can test only Granny's generic adapter mechanics and cannot prove a third-party service route.
- **Fixture/input after route selection:** reviewed named-provider SDK/service or a controlled Media3 fixture for adapter-only mechanics, synthetic test account where provider permits, two same-title items, exact stable URI, ad/paywall/unavailable states.
