...
**Device disposition, 2026-09-19:** three bounded, content-free [device runs](2026-09-19-t101-c2-device-evidence.md) used reviewed fixture and observer packages on `TBL-01`; each ended with both packages uninstalled and debugging restored. Across the repaired runs, C2-06's synthetic stale-prior-frame regression closed twice, visible Stop passed, prompt lock cleanup was observed and rotation produced recoverable explicit resize evidence. The latest C2-12 run ended sharing on task removal but reopened with the stale C2-10 result rather than a new task-removal result, so truthful result-state recovery still fails. The observations do not satisfy independent package/source, retention or egress oracles. Current API review leaves selected-package identity unresolved: MediaProjection supplies chosen content but the API 36 observer does not independently receive the selected package.
...
The latest C2 lifecycle repair passed 32 host cases and then ran under separate exact authority. It produced positive bounded Stop, lock, resize and task-removal projection-cleanup observations, but C2-12 exposed a lower-generation activity restart that left stale C2-10 result text visible. A source-only generation-seeding repair now passes 33 host cases but is unrun on Android and does not revise C2-12. Selected-package identity and independent retention/egress evidence remain unresolved. C5 is lower risk but does not reduce the two-external-workflow kill risk. Each other C1–C5 experiment still requires its own device/build/fixture/account/grant/network/actuation/retention authority. T-104 remains blocked until the sent-photo route and applicable device-safety boundary have actual evidence.
