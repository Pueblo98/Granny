...
Access dates below are 2026-09-19, with Android common intents and Spotify policy rechecked on 2026-09-20. Page versions are recorded when the publisher exposes one; an access date is not an effective date.
...
| Android Developers, [common intents](https://developer.android.com/guide/components/intents-common) and [Sharesheet guidance](https://developer.android.com/distribute/aep/aep-req-share-sheet) | `ACTION_SENDTO` can initiate SMS/MMS or email compose. Separately, `INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH` is documented for voice-command media requests; the sender supplies a media focus plus required artist/title metadata and the receiving app searches its inventory and starts playback. | Local code can package-target an exact compatible handler after Granny interprets and previews the request. Intent resolution or `startActivity` return proves only a handoff, not which item played, active playback or Pause. Provider/app terms and actual handler behavior remain separate evidence. |
...
| Spotify for Developers, [Android SDK](https://developer.spotify.com/documentation/android), [official SDK release](https://github.com/spotify/android-sdk/releases/tag/v0.8.0-appremote_v2.1.0-auth), [Developer Policy](https://developer.spotify.com/policy) and [Developer Terms](https://developer.spotify.com/terms) | The beta Android SDK can initiate/control playback and subscribe to `PlayerState` after app authorization. App Remote 0.8.0 / Auth 2.1.0 remains the listed release. The current policy prohibits voice-enabled streaming developer applications that let a user control Spotify by voice or voice assistants that provide voice control. | Technically plausible but **policy no-go for Granny's accepted voice-first media workflow** absent explicit written provider permission or a material product/scope change. No install, account or device experiment should start from this route. |
| Apple, [MusicKit](https://developer.apple.com/musickit/), [Android authentication](https://developer.apple.com/documentation/applemusicapi/user-authentication-for-musickit) and [Developer Program License Agreement](https://developer.apple.com/support/terms/apple-developer-program-license-agreement/) | Apple publishes Android authentication and playback libraries for in-app Apple Music songs/albums/playlists. User and developer tokens are required; current terms require user-initiated playback, standard media controls and prohibit charging or indirectly monetizing access to Apple Music. | A **desk lead only**, not a selected route. Android SDK currency, Android distribution eligibility, subscription/test-account availability, voice-assistant compatibility, privacy/security and commercial fit need authoritative review before dependency or account work. |
| Google, [YouTube on Android](https://developers.google.com/youtube/android), [IFrame Player API](https://developers.google.com/youtube/iframe_api_reference) and [required minimum functionality](https://developers.google.com/youtube/terms/required-minimum-functionality) | The former native Android Player API is no longer supported; current Android integration points to the embeddable IFrame player. It can control embedded playback and report state while retaining required YouTube identity, controls and policy behavior. | Potential in-app video playback lead, not control of the YouTube Music app and not yet an accepted music-service route. Web/network data flow, autoplay/user-action limits, provider UI and policy need separate review. |
...
| Route / build | Grants, auth and consent | Observation → action → independent postcondition | Stop/takeover and collateral effects | Version/distribution constraint | Evidence / disposition |
|---|---|---|---|---|---|
| Package-targeted Android `INTENT_ACTION_MEDIA_PLAY_FROM_SEARCH` / synthetic evidence lab | Installed receiving app and its own account/auth; Android package visibility limited to the exact intent action. Granny discovers compatible handlers locally and the user chooses when more than one exists. | Bind exact artist/title focus and query to one reviewed component, show exact preview, then call `startActivity`. A successful return proves only handoff acceptance. Without owner session/player evidence, report `playback requested`, never matching content or playing. | Stop before dispatch revokes the one-use request. After external launch, Granny cannot generically stop/pause or reverse it; user takes over in the receiving app. Queue replacement, ads, auth and app focus remain possible. | Handler presence and interpretation vary by app/version/device. The platform contract does not establish provider-policy compatibility or a stable postcondition oracle. | **Safe partial on TBL-01:** [T-124's YT Music run](2026-09-20-t124-media-handoff-device-evidence.md) verified exact handoff but the selected session remained paused. **No-go as CAP-10 playback completion** pending provider policy, exact-content/playing evidence and Pause. |
...
| Spotify Android SDK App Remote 0.8.0 + Auth 2.1.0 | Technically requires Spotify app, authorization and authenticated eligible account | Its API could resolve URI/play and expose `PlayerState`; that technical surface is not permission to use it for this product | Stop/Pause and all provider/account collateral effects would still require proof | Current Spotify policy prohibits the accepted voice-assistant control behavior | **No-go for current Granny scope** absent written provider permission or a material product change; do not install or account-test |
| Apple MusicKit for Android / desk lead | Apple developer/media identifiers and keys, Apple Music user authentication and eligible subscription; exact Android distribution terms unresolved | In-app catalog/library resolution and playback appear technically available; exact media ID/state and pause would need an independent oracle | Standard playback controls are required; subscription/auth/catalog/region and background playback are collateral conditions | Published Android docs exist, but SDK currency, Android commercial/distribution fit and voice-assistant use require specialist review | **Unknown/conditional lead**; no dependency, account or device action authorized |
...
**Workflow disposition:** Android's documented play-from-search intent is now a bounded **handoff experiment** that does not require a provider SDK. It is suitable for source/host implementation and later exact-handler device inspection, but it cannot complete PRD-FR-010 or CAP-10 because launch is not independent playback evidence and generic Pause is absent. Spotify's SDK remains policy no-go for the accepted voice-first behavior; whether a Spotify app receiving a platform intent is permitted is unresolved and must not be inferred. Apple MusicKit remains a desk lead. A named handler/version, provider-policy review and independent player/session oracle are still required before candidate admission.
...
| Requested media | Android package-targeted play-from-search **safe partial for YT Music handoff / No-go as completion**; Spotify SDK **No-go**; Apple MusicKit/owner session **Unknown/Conditional** | Standard-intent handoff or fixed recipe **Conditional** | No — one handoff reached YT Music but its session remained paused; no content/playing oracle or Pause |
| Granny-owned text | Own-app preference **Go** | Not needed | No — internal setting |

The standard Android intent removes the earlier assumption that a media experiment must begin with a provider SDK. The kill criterion is **not resolved**: scoped screen explanation is not admitted, the T-124 YT Music run proved only an external handoff and independently observed a paused session, sent-photo retrieval has no present candidate route, message preparation/handoff cannot count as send and text adjustment is internal. The next media decision is whether provider review plus a non-personal identity/playing oracle can explain and overcome the paused result. If not, Simon must still choose a policy-compatible owner integration, revise the outcome, replace a workflow or revisit ADR-0009's bar.
...
### C4 — media handoff and named integration (one safe-partial device attempt)

- **Execution status:** source/host implementation is complete and [one exactly authorized TBL-01 attempt](2026-09-20-t124-media-handoff-device-evidence.md) ran against preinstalled YT Music using the synthetic artist. Exact target handoff was observed, but the package-matched session remained paused at both checks. No account state, catalog metadata, personal content or screenshot was inspected. EVAL-004 remains incomplete; no further device/provider action is authorized by that completed run.
- **Fixture/input for the handoff phase:** exact synthetic artist `Elton John`; zero, one and two fake compatible handlers; stale/removed handler; launch exception; Stop before dispatch; candidate-build denial. A later separately authorized device phase must name the installed handler/package/version and use synthetic/non-personal content under reviewed provider terms.
- **Handoff expected effect:** exact artist focus/query and exact selected component survive preview and one-use dispatch; `startActivity` acknowledgement yields only `playback requested`. No handler yields known no effect; exception yields unknown with no retry.
- **Completion-phase fixture after route admission:** reviewed named owner/session integration or independent player oracle, synthetic test account where provider permits, two same-title items, exact stable URI, ad/paywall/unavailable states.
