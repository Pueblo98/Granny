...
**Device disposition, 2026-09-19:** two bounded, content-free [device runs](2026-09-19-t101-c2-device-evidence.md) used reviewed fixture and observer packages on `TBL-01`; each ended with both packages uninstalled and debugging restored. The repaired run returned the expected C2-02–05 categories, closed the synthetic C2-06 stale-prior-frame regression twice and demonstrated the observer's visible Stop for C2-08. It did not establish admission: prompt lock cleanup did not pass C2-09, C2-10 produced no recoverable explicit resize result, and C2-12 failed because capture outlived removal of the observer task and its reopened UI state until manual Stop. The observations still do not satisfy independent package/source, retention or egress oracles. Current API review leaves selected-package identity unresolved: MediaProjection supplies chosen content but the API 36 observer does not independently receive the selected package.
...
The repaired C2 artifacts passed 29 host cases and a separately authorized device run. That run closes only the bounded protected-scene freshness regression and demonstrates the visible Stop control. A second source-only repair now adds explicit screen-off cleanup, explicit task-removal cleanup and content-free result/session-state recovery across activity recreation; 32 host cases pass, but those changes are unrun on Android and do not revise the device ledger. Selected-package identity and independent retention/egress evidence remain unresolved. C5 is lower risk but does not reduce the two-external-workflow kill risk. Each other C1–C5 experiment still requires its own device/build/fixture/account/grant/network/actuation/retention authority. T-104 remains blocked until the sent-photo route and applicable device-safety boundary have actual evidence.
