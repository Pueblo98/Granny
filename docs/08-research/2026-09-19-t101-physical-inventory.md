...
The next useful thesis-risk experiment is the planned one-session scoped screen explanation (`C2`), because it tests one of only two still-plausible external workflow families without requiring personal accounts. Its [lab-only source scaffold](../../experiments/c2-screen-explanation/README.md) is now prepared but unbuilt and unrun. It still requires separate authority for the official host toolchain/dependency downloads, then a later exact device-run authorization for reviewed APKs, USB/ADB/debugging changes, install/teardown, per-session capture consent and C2-01–C2-12. Until those steps are authorized, installs, capture, permission changes, app actuation and network/egress remain prohibited.
