...
The next useful thesis-risk experiment is the planned one-session scoped screen explanation (`C2`), because it tests one of only two still-plausible external workflow families without requiring personal accounts. It cannot run yet: it requires separate authority to create and install a local synthetic screen fixture plus a bounded test build, show the system's per-session capture consent, actuate only synthetic screens, exercise deny/revoke/Stop/secure-screen variants and remove test data under an agreed teardown. Until that exact authority is granted, Android scaffolding, installs, capture, permissions, app actuation and network/egress remain prohibited.
