...
| POL-01 Navigate/reversible | Execute admitted low-consequence step for current intent | No repetitive confirmation; unlocked session and actual grant check | Visible task + Stop; no undo claim for navigation with discarded work; detect unsaved edits first | 12 actions/60s task cap; one no-effect retry; postcondition must verify; unknown surface denied |
