...
Drafts are temporary synthetic files, removed on graceful MCP shutdown; crashes may leave synthetic temporary files. Backend conversation and event history are process memory only and bounded; session expiry invalidates authority. For review of the local browser interaction, starting a new conversation also snapshots the prior visible transcript in browser-tab memory so **Today** can reopen it. It is never written to `localStorage` or `sessionStorage`, reload clears it, and **Clear history** removes prior tab-memory transcripts while retaining the active conversation. No production retention/encryption/deletion or personal-data suitability is claimed. Diagnostics omit transcripts, raw tool/provider errors and credentials. This scoped experiment changes the prior blanket no-backend/no-egress prototype boundary only as explicitly requested; real personal data, accounts, capture and Android remain excluded.
