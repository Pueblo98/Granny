...
Simon authorized the [local conversation/MCP demo](../04-architecture/conversation-runtime-contract.md) with fictional contacts and unsent local drafts. Scripted and connected demo modes have no provider egress. Live mode requires both server opt-in and explicit UI synthetic-text consent; bounded conversation text and, inside a Room, at most three relevant non-private current-Room fictional references reach OpenRouter under the inherited Qwen privacy/price filters. Home sends no Room references. This slice does not perform cross-room retrieval, whole-Room export or real-document ingestion. Keys stay server-side and are omitted from the MCP child environment. Model proposals never receive confirmation authority or MCP results.

Drafts are temporary synthetic files, removed on graceful MCP shutdown; crashes may leave synthetic temporary files. The separately authorized conversation/evidence slice persists synthetic conversations in a bounded versioned SQLite file at a server-fixed OS-temporary path. Page reload and backend restart retain completed transcripts, exact source revisions and message-specific evidence links; runtime permits, pending actions and MCP draft authority do not survive restart. The browser cannot choose a path or issue database queries, and no transcript is written to `localStorage` or `sessionStorage`.

Starting a new conversation creates a new ID/runtime session and archives the prior transcript for read-only Today review. **Clear history** removes archived nonfixture conversations while retaining the active conversation and fixed labelled samples. **Reset everything** removes all nonfixture conversations and minimal action history, restores deterministic fixtures and creates a fresh offline conversation. The synthetic bound is 20 nonfixture conversations, 200 messages per conversation and 100 action-history rows. These are implementation limits, not production retention, encryption, backup or deletion promises; OS temporary-file cleanup is not guaranteed. Diagnostics omit transcripts, rejected private content, raw tool/provider errors and credentials. [The store contract](../04-architecture/conversation-evidence-store.md) owns exact lifecycle and recovery. Real personal data, accounts, capture and Android remain excluded.
...
Home composition follows [ADR-0016](../09-decisions/ADR-0016-explicit-home-room-row.md). Its optional continuation may reference only data and capabilities already admitted for the active release. Neither continuation nor the room row shows message bodies, contact details, health/financial facts, private-memory values or another person's content unsolicited; use a generic description and explicit open action. Room portraits/names expose only admitted room metadata, never sensitive contents. Hide removes the current continuation from Home selection, and Reset Home clears Home pins/suppression state without deleting the underlying task, room, artifact or memory. No panel/row impression, click or absence becomes advertising, profiling or family/helper telemetry.
...
T-119 uses deterministic fictional references and in-memory membership only. Its design/runtime mock must have no personal files, browser persistence or implied production encryption. The only provider-egress exception is the explicitly consented 2026-09-20 live synthetic slice: at most three non-private, non-excluded references from the current Room, never cross-room or whole-Room content. Proposed App V1 storage remains blocked until a storage/backup/tombstone design and EVAL-008/012 evidence are reviewed.
