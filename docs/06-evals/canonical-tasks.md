...
- **Failure conditions:** Silent inference/transcript archive; resurrected data; raw diagnostics; coerced helper access; unconsented egress; secret in export; room membership widens authority/sharing; model-only findability; deletion of a room silently deletes or strands underlying data.
...
- **Happy path:** User selects voice/touch/guidance, completes workflows, explains action before commit and result after, finds Stop, understands or dismisses continuation and states desired control. In the Context Rooms study, the participant uses the selected Explicit Scroll Row and written controls, enters a labeled room, explains that Granny is the same assistant, identifies current/cross-room sources, finds an item without chat and returns Home for a global task.
- **Adversarial/failure variants:** Wrong suggested person, irrelevant/stale/sensitive continuation, continuation absent, partial outcome, timeout, noisy room and family absent. The Rooms extension covers CMP-012 no-room/one-room/all-fit/overflow, false overflow controls, portrait failure, large-text vertical fallback, misleading atmosphere, item in two rooms, wrong cross-room result, model offline, room absent and delete-room-versus-delete-data comprehension.
...
- **Failure conditions:** Undisclosed fake send; coercion; inaccessible consent; confusion about actual outcome or whether continuation/room selection is required; repeated Home distraction or inability to find Talk/Type; room navigation depends on an undisclosed gesture; participant believes rooms are different agents or security boundaries; cannot find an item without conversation; misunderstands room deletion; researcher coaching counted as independent completion.
