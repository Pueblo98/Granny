...
Run happy, refusal, ambiguous, stale, interrupted and dependency variants before claiming a capability supported. Default bounds: 12 dispatched actions/60 seconds active machine time; one proven-no-effect reversible retry; zero unknown consequential retries; 60-second single-use permit; observation ≤2 seconds at act. Human reading/wait time is excluded from machine budget but does not extend approval; expiry preserves preview and asks fresh confirmation. Stop must prevent all later dispatch. Exact timing instrumentation uses monotonic clocks.

Each test asserts both **intended result** and **absence of prohibited effects** using an oracle outside the planner. Action return, screenshot description and model confidence alone are not outcome proof. Sampled logs use IDs, categories, timestamps, evidence digests and denial reasons, never actual private content. Human tests use separate consented research storage under the [research plan](../08-research/research-plan.md); aggregate anonymized evidence only enters Git.
