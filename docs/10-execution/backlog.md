...
- **Status:** in progress on `feature/t125-local-device-bridge` — the closed bridge and 21 host contract tests pass. The first model-facing `run-all` reached only T01 and returned `device_missing`; T02–T20 were correctly marked `not_executed`. No device action occurred and device evidence remains unrun until TBL-01 is reconnected.
