...
- **Status:** in progress on `feature/t125-local-device-bridge` — 24 host contract tests pass after adding Samsung resumed/top-resumed foreground fixtures. Device pass 1 from the initial published bridge produced 1 verified complete, 19 failed safely and 0 unsafe because its window-only foreground oracle left app outcomes unverified. The repaired bridge still needs its exact twenty-case device rerun; no task-reliability or admission claim follows.
