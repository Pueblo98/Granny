...
- **Status:** review — complete local demo slice and live-consent wiring implemented. The 2026-09-20 bounded extensions carry the same assistant across Home and all six fictional Rooms, validate at most three current-Room candidates and persist separate conversations, messages, immutable source revisions, retrieval/provider runs and exact message-evidence links in backend SQLite. The provider must declare the source IDs actually used; the UI fetches citations by exact assistant message ID. New conversation creates a fresh ID/session, prior Today rows open their exact read-only transcript, and labelled fictional samples cannot alias the active conversation. One separately run live synthetic Room question returned a grounded answer with the expected source binding and no draft write. [Original session](sessions/2026-09-15-mcp-backend-integration.md), [Room extension](sessions/2026-09-20-room-chat-openrouter.md) and [evidence-store session](sessions/2026-09-20-conversation-evidence-store.md) own exact checks and limitations. This does not prove general live-model quality or promote a product gate.
- **Excluded / rollback:** No external send/account, personal files/data, production retention/database claim, Android, root, remote server install, shell/network tool or broad capability expansion. Stop server to disable; graceful shutdown removes the MCP synthetic draft store and closes the bounded conversation database without restoring pending authority. Use the documented reset/file rollback for synthetic transcripts or revert the scoped branch through normal reviewed Git.
...
- **Scope / authority:** Simon adopted Context Rooms on 2026-09-19 and separately authorized the selected Home frontend. During that implementation he explicitly extended the fictional browser checkpoint to six starter rooms, all 48 collection symbols, direct library/search/detail, source-aware fixture replies and bounded explicit room creation, using the Kitchen vertical-slice mockups. He later authorized bounded local synthetic conversation/evidence persistence. [Current session](sessions/2026-09-19-context-rooms-frontend.md) owns the design extension; production persistence, production context and archive/delete contracts are not admitted.
...
- **Status:** in progress — the selected Home, six-room frontend, in-memory membership receipts/Undo, archive/restore, separated deletion scopes, degraded states and supporting navigation are integrated on `main` through PR #44. The unmerged Room/OpenRouter line now has one Home/Rooms assistant plus a server-owned versioned SQLite store for bounded synthetic conversations and exact answer-owned evidence. Deterministic repository/API/browser checks cover message-specific citations, source revision/deletion truth, future-only preferences, restart, Today isolation and failure/replay cases. Full T-119 remains incomplete: production typed-context/privacy enforcement, real cross-room admission, Android storage and representative human/device evidence remain unrun. Room organization mutations themselves remain browser-memory fixtures.
- **Excluded / rollback:** No personal data, browser persistence, production device persistence, provider egress beyond the explicitly consented bounded synthetic Room context, sharing/helper access, production database, Android automation, separate room agents, cognitive/dementia benefit claim or MVP expansion. Disable live mode, reset/remove the fixed synthetic temporary database, or revert the task branch without changing scripted Home/Rooms.
