...
- **Status:** partial handoff experiment in review through [T-124](backlog.md#t-124) — an exact TBL-01/YT Music attempt reached the selected component and exposed its media session, but the session remained paused. It did not verify matching content or provide Pause, so T-106 completion remains blocked on provider-policy review, a non-personal identity/playing oracle and explanation of the paused result. Spotify's SDK route remains policy no-go. No further account/device work is authorized.
...
- **Dependencies / entry:** Preserve ADR-0016's selected Explicit Scroll Row and the Round conversation shell; reuse registered components; no real storage/provider/device route. Read the [selected Home reference](../02-design/mockups/2026-09-19-harbour-blue-home/selected-explicit-scroll-row/README.md), [task packet](task-packets.md#t-119-packet), [Context Rooms](../02-design/context-rooms.md), [reviewed-pack visual system](../02-design/context-room-visual-system.md) and action/privacy policy.
...
- **Status:** review on `feature/t124-media-play-search` — 161 host cases across 17 suites, debug assembly and lint pass; the runtime dependency graph is empty and the APK declares only `RECORD_AUDIO`. One [TBL-01/YT Music attempt](../08-research/2026-09-20-t124-media-handoff-device-evidence.md) verified exact handoff and honest return handling, but the selected session remained paused. This is safe partial device evidence, not CAP-10 playback admission.
...
<a id="t-125"></a>
## T-125 — Run a closed twenty-case local device bridge experiment

- **Scope / authority:** Simon explicitly requested trying twenty representative tasks on the connected development tablet before moving the controller to a Raspberry Pi, without exposing raw ADB to the model. Implement a host-only lab bridge whose public CLI accepts only `catalog`, one named `T01`–`T20` case or the complete named suite.
- **Trace / owner:** PRD-FR-004/010/012/013, PRD-SAF-001/003/004/005, PRD-NFR-001/003/004; UC-004/011/017/019; CAP-01/03/04/10/12 boundary themes; POL-00/01/R; EVAL-004/006/008/009/010; THR-02/05/06/08/13/16/19.
- **Dependencies / entry:** Stack on T-124's exact media fixture and T-103's authority lessons. Use exactly one already-authorized, awake and unlocked `TBL-01`; current per-run authority covers fixed public Granny/Spotify/YouTube/browser/settings/Clock tasks only. No new app, account, provider SDK, permission or device configuration.
- **Deliverables:** Standard-library host bridge; exact twenty-case catalog; fixed app/text/URL/system-page registry; semantic-node-derived taps; exact foreground/media identity/state oracles; normalized output without device IDs, raw UI/XML/media metadata or transport errors; contract tests; deidentified evidence record after an actual run.
- **Acceptance:** Unknown task/app/target/text/page and component mismatch deny before action; exactly one ready device is required; no public interface accepts a shell command, ADB argument, package, coordinate, arbitrary text/URL/intent or screenshot export; all twenty cases receive one terminal classification and unrun dependencies remain `not_executed`; Spotify is paused at teardown; no unsafe effect or retained personal content.
- **Status:** in progress on `feature/t125-local-device-bridge` — 26 host contract tests pass. Pass 1 produced 1 verified complete / 19 failed safely / 0 unsafe. Pass 2 reported 10/10/0, but evidence review downgrades T15 because `resultObserved=false`; interpreted result is 9 verified complete / 11 failed safely / 0 unsafe. The bounded cold-start and fail-closed YouTube-oracle repair still needs a device rerun; no task-reliability or admission claim follows.
- **Excluded / rollback:** This is not a production capability, Play-policy approval, hard host security boundary, Pi implementation, dynamic model click tool, accessibility-service grant, screen stream, raw shell, arbitrary package/text/URL/coordinate, message/contact/file task, credential/account view, purchase, install, permission/security mutation or gate pass. Delete `experiments/local-device-bridge` to remove the harness; third-party public search/play history from a later successful run is not silently deleted.
