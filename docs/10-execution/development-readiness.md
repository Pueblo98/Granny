...
| GATE-03 Android control feasibility | Exact device/app matrix, API/semantic route, fresh targeting, Stop and independent postconditions; Android + safety review | Needs evidence; the repaired [C2 device run](../08-research/2026-09-19-t101-c2-device-evidence.md) closes the bounded protected-scene regression and demonstrates visible Stop, but prompt lock cleanup does not pass, rotation is inconclusive, task-removal containment fails and selected-package identity is unresolved | Only demonstrated synthetic device/control capabilities; unsupported routes stay disabled |
...
| GATE-06 Safety/privacy readiness for prototype | Reviewed finite tools, local permit/cancel/journal, threat/privacy contract; deterministic invariants then device isolation/grants/egress tests for actuation; safety/privacy reviewer + Simon for real data | Ready with proposed assumptions for **offline fake replay only**; C2 denial/withholding and visible Stop have narrow positive observations, but task-removal containment fails, prompt lock cleanup is absent and device retention/egress isolation remain unverified. Needs evidence for device actuation/real data | Synthetic contract replay now under DoR; actual device actuation only after invariants and platform controls verified; real data additionally terms/consent/support review |
...
- **Next dependency-ordered backend/evidence work:** [T-101](backlog.md#t-101) has Step A/C plans, a partial Step B inventory and two bounded [C2 device runs](../08-research/2026-09-19-t101-c2-device-evidence.md). A second source-only repair adds explicit screen-off/task-removal cleanup and content-free state/result recovery with 32 passing host cases; review its commit and exact artifacts before any fresh device authorization. Host evidence does not change the failed/inconclusive device rows. T-104 waits for sent-photo route/device evidence and the applicable device-safety boundary.
