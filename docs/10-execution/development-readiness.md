...
- **Next product-app discussion:** no frontend revision is currently queued. Select and prepare one bounded native slice before code: the leading candidates are setup/capability status, a local fictional draft/clarification/preview flow, or recent activity after its storage/backup/retention contract. This discussion does not itself admit a task or widen external capability scope.
