...
| GATE-03 Android control feasibility | Exact device/app matrix, API/semantic route, fresh targeting, Stop and independent postconditions; Android + safety review | Needs evidence; T-101 has a partial `TBL-01` configuration and an unbuilt [C2 source scaffold](../../experiments/c2-screen-explanation/README.md), but host build validation, selected-package identity, every device postcondition and Stop/takeover trial remain unrun | Only demonstrated synthetic device/control capabilities; unsupported routes stay disabled |
...
- **Next dependency-ordered backend/evidence work:** [T-101](backlog.md#t-101) has Step A/C plans, a partial Step B inventory and a [C2 lab source scaffold](../../experiments/c2-screen-explanation/README.md). Separately authorize official JDK/Android SDK/Gradle/ADB setup and dependency downloads to build/test the exact scaffold. Device installation, debugging changes, capture consent and C2-01–C2-12 remain a later exact authorization after artifact/oracle review. T-104 waits for sent-photo route/device evidence and the applicable device-safety boundary.
