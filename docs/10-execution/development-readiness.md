...
| M6 App V1 | Broader coherent release justified by MVP learning | Pilot review + separate V1 scope and per-extension gates | Calls/basic reminders/document reading/ADR-0012 automatic important facts and adaptive communication/helper proposals where validated; updated adapters/evals/support | EVAL-013–017 and all inherited MVP invariants; Simon release decision with market-specific review | Remote screen/control, transcript/story capture, proactive routines unless separately accepted; all Stage 2/3 |
...
- **Next dependency-ordered work:** [T-101](backlog.md#t-101) Step B read-only physical inventory after explicit tablet/access authorization, followed only by separately authorized Step C experiments. [Step A and both worksheets](../08-research/2026-09-19-t101-route-inventory.md) are prepared; device evidence remains unrun. T-104 waits for sent-photo route/device evidence and the applicable device-safety boundary.
