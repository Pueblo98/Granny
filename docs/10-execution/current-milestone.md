...
T-101 remains in progress; GATE-03/04 and GATE-06 device/real-data conditions remain open. The [lab-only Android 16 scaffold](../../experiments/c2-screen-explanation/README.md) passed its host checks and then had one bounded [C2 device run](../08-research/2026-09-19-t101-c2-device-evidence.md). Narrow denial/classification/withholding/invalidation observations do not overcome a repeated protected-content failure, an aborted Stop attempt after temporary tablet unresponsiveness, inconclusive/unrun lifecycle cases or the unresolved API 36 selected-package oracle. Both packages were removed and debugging restored. T-104 still waits for actual sent-photo route and device-safety evidence.
...
Evidence-led foundation: MVP/control review T-100 and the bounded offline T-103 core are complete. T-101 Step A/C plans, partial Step B inventory and the first C2 device ledger now exist. Next, separately authorize a source-only C2 repair/review for the protected-content and lifecycle harness before any fresh device request; do not rerun the current APKs. T-104 waits for sent-photo route/device evidence. Produce/test low-fi interaction and iterate naming/identity (T-102/108) within their authorization and evidence boundaries. Do not begin broad MVP implementation or personal-data automation.
