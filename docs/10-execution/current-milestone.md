...
T-101 remains in progress; GATE-03/04 and GATE-06 device/real-data conditions remain open. The [lab-only Android 16 scaffold](../../experiments/c2-screen-explanation/README.md) now has two bounded [C2 device runs](../08-research/2026-09-19-t101-c2-device-evidence.md). The repaired run closes the synthetic protected-scene regression twice and demonstrates visible Stop, but prompt lock cleanup does not pass, rotation lacks a recoverable result and task-removal containment fails. A second source-only repair adds explicit screen-off/task-removal cleanup and content-free state/result recovery and passes 32 host cases; it is unrun on Android and does not change those dispositions. Selected-package identity and independent retention/egress evidence remain unresolved. Both device-run packages were removed; debugging authorization, USB debugging and Developer options were disabled, and the cable was disconnected. T-104 still waits for actual sent-photo route and device-safety evidence.
...
Evidence-led foundation: MVP/control review T-100 and the bounded offline T-103 core are complete. T-101 Step A/C plans, partial Step B inventory and two bounded C2 device runs now exist. The next C2 lifecycle/state repair passes host checks; review its commit and exact APK hashes before deciding whether to authorize another tablet run. Any tablet access still requires fresh exact authority. T-104 waits for sent-photo route/device evidence. Produce/test low-fi interaction and iterate naming/identity (T-102/108) within their authorization and evidence boundaries. Do not begin broad MVP implementation or personal-data automation.
