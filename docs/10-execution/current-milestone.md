...
T-101 remains in progress; GATE-03/04 and GATE-06 device/real-data conditions remain open. Simon authorized C2 source preparation, and the [lab-only Android 16 scaffold](../../experiments/c2-screen-explanation/README.md) now defines separate synthetic fixture/observer apps, no-network/no-retention boundaries, pure state/interpreter tests and the exact later build/install/run/teardown plan. It is unbuilt and unrun because no host Android toolchain or dependency/device authority exists. API 36 also leaves selected-package identity unresolved, so a fixture marker cannot prove the full source oracle. T-104 still waits for actual sent-photo route and device-safety evidence.
...
Evidence-led foundation: MVP/control review T-100 and the bounded offline T-103 core are complete. T-101 Step A/C plans, partial Step B inventory and unbuilt C2 source scaffold now exist; all runtime cases remain unrun. Next, separately authorize the official Android host toolchain/dependency setup and build verification. Device install/capture remains a later exact authorization after APK/diff review and package-identity-oracle assessment. T-104 waits for sent-photo route/device evidence. Produce/test low-fi interaction and iterate naming/identity (T-102/108) within their authorization and evidence boundaries. Do not begin broad MVP implementation or personal-data automation.
