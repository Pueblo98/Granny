...
T-101 remains in progress; GATE-03/04 and GATE-06 device/real-data conditions remain open. Simon authorized C2 source preparation and host-only build verification. The [lab-only Android 16 scaffold](../../experiments/c2-screen-explanation/README.md) now builds separate synthetic fixture/observer debug APKs, passes 10 pure host tests and lint with zero errors, and records exact hashes and no-backup/no-egress boundaries. No APK was installed and no device case ran. API 36 also leaves selected-package identity unresolved, so a fixture marker cannot prove the full source oracle. T-104 still waits for actual sent-photo route and device-safety evidence.
