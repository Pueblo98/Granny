...
public final class ConversationSpeechBridgeTest {
    ...
    private static final class Adapter implements SpeechOutputAdapter { Availability state=Availability.AVAILABLE; boolean accept=true; int calls,stops; String text; long generation; Listener listener; public Availability availability(){return state;} public boolean speak(long g,String t,float r,Listener l){calls++;generation=g;text=t;listener=l;return accept;} public void stop(){stops++;} public void destroy(){} void complete(){complete(generation);} void complete(long g){listener.onCompleted(g);} }
