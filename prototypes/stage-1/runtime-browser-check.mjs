...
try {
  ...
  await b.navigate(); await wire(); await b.evaluate('window.__wire.liveAvailable = true');
  await b.click('#open-kitchen'); await b.click('#collection-recipes');
  await b.click('[data-item-id="vegetable-soup"]'); await b.click('#ask-room-item');
  const directQuestion = await b.evaluate('document.querySelector("#request").value');
  await b.click('#composer button[type=submit]');
  await b.waitFor('document.querySelector("#confirm-dialog")?.open');
  check((await text()).includes('will go to OpenRouter/Qwen'), 'first recipe question opens provider consent instead of a scripted reply');
  check(await b.evaluate('window.__wire.requests.some(r=>r.url.endsWith("/config")) && window.__wire.requests.every(r=>!r.body?.mode && r.body?.kind !== "turn")'), 'first question checks availability but creates no session or model turn before consent');
  await b.click('#confirm-dialog button[value=confirm]');
  await b.waitFor('window.__wire.requests.some(r=>r.body?.kind === "turn")');
  const directRequest = await b.evaluate('window.__wire.requests.findLast(r=>r.body?.kind === "turn").body.payload');
  check(directRequest.text === directQuestion && directRequest.context.place.roomId === 'kitchen' && directRequest.context.sources.some(source=>source.itemId === 'vegetable-soup'), 'consent connects and sends the original recipe question with its bounded source context');
  check(!await b.evaluate('!!document.querySelector(".room-reply")'), 'live-available recipe input never falls through to the scripted Room responder');
