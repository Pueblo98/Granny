...
**Menu → Settings → About Granny → AI connection** checks the same-origin loopback runtime automatically. When the server was started with `--live` and a valid private key, **Use live AI chat** is the primary action and opens the separate fictional-data/provider-egress consent. **Use offline test replies** is visibly identified as a fixed-response stub for local draft-flow testing, not an AI assistant. The static server above deliberately has no API, so either option requires the loopback runtime server to serve these current frontend assets. Its canonical interface is [granny.conversation.v1 at the agreed checkpoint](https://github.com/Pueblo98/Granny/blob/aea7f442021347aab0130fcbe220ab5a1ab53b1b/docs/04-architecture/conversation-runtime-contract.md); frontend does not own or duplicate that runtime. See the [frontend coordination record](../../docs/10-execution/sessions/2026-09-15-ui-backend-frontend.md) for exact integrated commits and evidence before claiming a runnable combined build.
...
Text size and unfinished composer/editor content are preserved across secondary navigation. Scripted aliases, music and activity remain separate from backend contacts/execution. Opening AI connection performs only the automatic same-origin availability check; no session or model call occurs before consent. No key is sent to the browser, and CSP permits same-origin connections only. The default five-workflow scripted experience is unchanged.
...
Under Simon's backend integration mission and 2026-09-20 Room extension, Menu → Settings → About Granny → AI connection automatically checks live availability. Only an explicitly enabled backend with `liveAvailable` exposes the primary **Use live AI chat** action. Separate consent names synthetic conversation and bounded current-Room reference egress to OpenRouter/Qwen; cancel makes no session, and confirming creates a mode-bound session without a model call. Text submission starts interpretation. Default remains scripted; **Use offline test replies** remains a clearly labelled fixed stub. Keys never enter the browser. No recording, account access, real sending, new capabilities or increased caps.
