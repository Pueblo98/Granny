...
- Menu reveals the secondary settings/help/privacy/history functions. Scripted data lives in this tab's memory; no browser persistence or account exists. Connected mode uses the server-owned bounded synthetic SQLite store: reload/restart preserves completed conversations and exact message citations, while runtime permits and pending actions do not survive. Do not enter personal data.
...
## Connected conversation runtime (separate opt-in)

On the loopback server, the first composer submission from Home, any Room or **Ask Granny about this** checks live availability. When the server was started with `--live` and a valid private key, the submitted words stay in the composer while the separate fictional-data/provider-egress consent opens. Continuing creates a live session and sends that same question with its bounded current-place context; cancel creates no session/model call and preserves the words. The input does not silently fall through to a scripted Room response when live AI is available. **Menu → Settings → About Granny → AI connection → Use live AI chat** remains an alternate entry. **Use offline test replies** is visibly identified as a fixed-response stub for local draft-flow testing, not an AI assistant. The static server above deliberately has no API, so it retains the scripted prototype after its local availability check fails closed. The runtime's canonical interface is [granny.conversation.v1 at the agreed checkpoint](https://github.com/Pueblo98/Granny/blob/aea7f442021347aab0130fcbe220ab5a1ab53b1b/docs/04-architecture/conversation-runtime-contract.md); frontend does not own or duplicate that runtime. See the [frontend coordination record](../../docs/10-execution/sessions/2026-09-15-ui-backend-frontend.md) for exact integrated commits and evidence before claiming a runnable combined build.

The connected slice uses one shared model conversation across Home, every Room, item-level **Ask Granny about this**, Help's **Ask Granny**, and Search's **Ask Granny instead**. These entries all converge on the same composer and runtime session; they do not create separate assistants or fall back to scripted Room replies while connected. **Start a new conversation** creates a fresh session in the already selected live/offline mode without another provider call. Full reset or explicitly returning to the scripted demo disconnects it. The runtime also prepares an unsent message for a fictional person: request, required clarification, exact person/channel/body/effect preview, explicit **Create this unsent demo draft**, creating/checking, and a verified **not sent** result. Editing message words creates a fresh preview; changing person/destination returns to a new natural request because backend-approved identity choices must not be invented by the frontend. An expired approval keeps the readable draft and offers review again. Typed “yes” is not confirmation.
...
Text size and unfinished composer/editor content are preserved across secondary navigation. Scripted aliases, music and activity remain separate from backend contacts/execution. Opening AI connection or submitting the first question performs only the same-origin availability check; no session or model call occurs before consent. No key is sent to the browser, and CSP permits same-origin connections only. The five-workflow scripted experience remains the fallback when the live backend is unavailable.
...
Under Simon's backend integration mission and 2026-09-20 Room extension, the first Home/Room/item question checks live availability and offers the same provider consent before creating a session or sending the preserved question. The AI connection screen remains an alternate explicit entry. **Use offline test replies** remains a clearly labelled fixed stub, and a server without live availability retains the scripted fallback. Keys never enter the browser. No recording, account access, real sending, new capabilities or increased caps.
