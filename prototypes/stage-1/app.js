...
(() => {
  ...
  let scheduler, pending = null, dialogReturn = null, menuPanel = '', lastAnnouncement = '', restoreFocus = false, editor = null, editingAliasId = null, panelReturn = null;
  ...
  function photoViewer(items, start, source) {
    ...
    const draw = () => {
      ...
      wrap.append(node('h2', '', item.title || 'Fictional photo'), node('p', 'notice', item.description || 'Fictional illustration.'), node('p', 'notice', [item.sender, item.detail, item.source, item.date].filter(Boolean).join(' · ')));
  ...
  function resultView(task) {
    ...
    if (task.kind === 'photos' && result.photos) {
      const gallery = node('div', 'photo-gallery'); result.photos.forEach((item, index) => { const b = button('', () => photoViewer(result.photos, index, b), 'photo-item'); const img = document.createElement('img'); img.src = item.asset || '/assets/garden.svg'; img.alt = item.alt || item.description || 'Fictional illustration'; b.append(img, node('strong', '', item.title || 'Open fictional photo'), node('span', 'notice', [item.sender, item.detail, item.source, item.date].filter(Boolean).join(' · '))); gallery.append(b); }); c.append(gallery); if (result.uncertainDate) c.append(node('p', 'notice', 'The date is uncertain in this fictional example.'));
  ...
  function render(editing) {
    ...
    if (state.task) {
      ...
      if (task.kind !== 'message' && task.stage === 'preview') { const slots = task.slots || {}; c.append(card('Check this step', task.kind === 'photos' ? [slots.person && slots.person.name, slots.person && slots.person.detail, 'Example Messages', slots.date].filter(Boolean).join(' · ') + '. Opening this fictional source may mark it read while the selected photos are checked.' : 'Nothing will happen until you choose Continue.')); }
    ...
    if (menuPanel === 'history') {
      ...
      history.append(button('Return to conversation', () => { menuPanel = ''; render(); panelReturn?.focus(); }));
    ...
    if (menuPanel === 'text') {
      ...
      const actions = node('div', 'inline-actions'); [1,1.15,1.3,1.5].forEach(scale => actions.append(button(Math.round(scale * 100) + '%', () => dispatch('setScale', scale), state.previewScale === scale ? 'primary' : ''))); settings.append(actions, button('Apply this size', () => dispatch('applyScale'), 'primary'), button('Restore previous size', () => dispatch('restoreScale')), button('Return to conversation', () => { menuPanel = ''; render(); panelReturn?.focus(); })); thread.append(settings);
    }
    if (menuPanel === 'help') { const help = card('What you can ask', 'You can ask to find fictional family photos, explain a supplied screen, play a fictional song, make Granny’s text easier to read, or open an unsent fictional message draft.'); help.dataset.panel = 'help'; help.append(button('Return to conversation', () => { menuPanel = ''; render(); panelReturn?.focus(); })); thread.append(help); }
    if (menuPanel === 'preferences') {
      ...
      (state.aliases || []).forEach(alias => { const edit = button(alias.label + ' — edit', () => { editingAliasId = alias.id; render(); }); edit.id = 'alias-edit-' + alias.id; const remove = button(alias.label + ' — remove', () => ask('Remove ' + alias.label + '?', 'This removes the name from this tab. It does not change a contact elsewhere.', () => { if (editingAliasId === alias.id) editingAliasId = null; dispatch('aliasDelete', alias.id); })); remove.id = 'alias-delete-' + alias.id; aliases.append(edit, remove); });
      const currentAlias = (state.aliases || []).find(alias => alias.id === editingAliasId); const label = document.createElement('input'); label.id = 'alias-label'; label.placeholder = 'Name, for example Sophie'; label.value = currentAlias ? currentAlias.label : ''; label.setAttribute('aria-label','New name');
      const person = document.createElement('select'); person.id = 'alias-person'; (window.GrannyFixtures?.people || []).forEach(p => { const option = node('option','',p.name + ' — ' + p.detail); option.value = p.id; option.selected = !!currentAlias && p.id === currentAlias.personId; person.append(option); }); person.setAttribute('aria-label','Person for this name');
      const save = button('Save name', () => { dispatch('aliasSave', { id:editingAliasId || undefined, label:label.value, personId:person.value }); editingAliasId = null; }); save.id = 'alias-save'; aliases.append(label, person, save);
      preferences.append(aliases, button('Return to conversation', () => { menuPanel = ''; render(); panelReturn?.focus(); }));
  ...
  function hasWork() { return !!(state.task && !['completed','unknown','stopped','failed','no-matches'].includes(state.task.stage)); }
  function clearLocalView() { composerText.value = ''; editor = null; menuPanel = ''; editingAliasId = null; }
  ...
  $('intro-talk').addEventListener('click', () => { dialogReturn = $('intro-talk'); $('talk-dialog').showModal(); });
  $('intro-skip').addEventListener('click', () => { $('introduction').hidden = true; composerText.focus(); });
  $('talk-dialog').addEventListener('close', () => { const result = $('talk-dialog').returnValue; if (result === 'use') newRequest($('talk-text').value); else if (result === 'cancel' && hasWork()) dispatch('stop'); else if (result === 'decline') composerText.focus(); else dialogReturn?.focus(); });
  ...
  document.querySelectorAll('[data-menu]').forEach(b => b.addEventListener('click', () => {
    ...
    if (what === 'return') { menuPanel = ''; render(); panelReturn?.focus(); return; }
    if (what === 'introduction') { $('welcome').hidden = false; $('introduction').hidden = false; $('menu').hidden = true; $('intro-talk').focus(); return; }
    if (what === 'new') { const fn = () => { clearLocalView(); dispatch('clearSession'); }; hasWork() || composerText.value.trim() ? ask('Start a new conversation?', 'Your unfinished request will be cleared. Text size and saved names stay in this tab.', fn, b) : fn(); }
    if (what === 'text' || what === 'history' || what === 'help' || what === 'preferences') { panelReturn = b; menuPanel = menuPanel === what ? '' : what; render(); }
  }));
