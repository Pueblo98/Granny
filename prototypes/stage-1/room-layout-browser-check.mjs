...
try {
  for (const [width,height,scale] of [[1440,900,1],[1440,1200,1],[1280,480,1],[840,600,1],[600,480,1],[360,640,1],[840,700,2]]) {
    ...
    if (width>700 && scale===1) check(await b.evaluate(`(()=>{const art=document.querySelector('.room-atmosphere'),end=parseFloat(getComputedStyle(art).getPropertyValue('--room-browse-start')),symbols=document.querySelector('.overview-symbols').getBoundingClientRect();return Number.isFinite(end)&&Math.abs(art.getBoundingClientRect().top+end-symbols.top)<1;})()`),'overview fade finishes before interactive collection symbols '+width+'x'+height);
    ...
    check(await b.evaluate(`(()=>{const c=document.querySelector('.room-conversation').getBoundingClientRect(),s=document.querySelector('#room-surface').getBoundingClientRect(),art=document.querySelector('.room-atmosphere');if(document.body.dataset.homeList==='true')return Math.abs(c.width-s.width)<1;const end=parseFloat(getComputedStyle(art).getPropertyValue('--room-reading-end'));return c.width<=s.width*.6&&Math.abs(art.getBoundingClientRect().left+end-c.right)<1;})()`),'chat reserves a clean left column or reflows to full width '+width+'@'+scale);
