...
try {
  ...
  check(await b.evaluate("(() => { const composer=getComputedStyle(document.querySelector('#composer')); const tail=getComputedStyle(document.querySelector('.composer-tail')); return composer.outlineColor==='rgb(73, 48, 161)' && composer.outlineOffset==='5px' && tail.pointerEvents==='none'; })()"), 'separated focus ring remains intentional and decorative tail is inert');
